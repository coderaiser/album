...
CloudServer._fillJSON = function(pStats, pFiles){
     ...
     */    
    if(CloudServer.NoJS){
        var lDir    = process.cwd() + '/',
            lAlbum  = cloudRequire(lDir + 'album'),
            lParse  = cloudRequire(lDir + 'parse');
        
        lList       = lParse.generateImgHTML(lAlbum);
...
CloudServer.indexReaded = function(pList){
    return function(pError, pIndex){
       ...
       /* если выбрана опция минифизировать скрпиты
        ...
        pIndex = pIndex.replace('<div class=covers></div>',
            '<div class=covers>' + pList + '</div>');
